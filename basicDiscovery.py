...
# file containing vehicle data
parser.add_argument("-d", "--data", action="store", dest="data", help="Filepath for vehicle data. Will be added row by row")
...
if (args.mode == 'publish'):
    ...
    vehicle_data = pd.read_csv(data_filepath)
    vehicle_num = int(data_filepath.split('/')[-1].split('.')[0][-1])
    # iterate over all rows in the file and send to lambda function
    for index, row in vehicle_data.iterrows():
        if args.mode == 'both' or args.mode == 'publish':
            message = {}
            message['timestep_time'] = row['timestep_time']
            message['vehicle_CO'] = row['vehicle_CO']
            message['vehicle_CO2'] = row['vehicle_CO2']
            message['vehicle_HC'] = row['vehicle_HC']
            message['vehicle_NOx'] = row['vehicle_NOx']
            message['vehicle_num'] = vehicle_num
